...
const starterMessages = [
  {
    role: "tutor",
    text: "Hey! I'm your AI tutor. Ask me anything about Decision Trees, KNN, Linear Regression, or Neural Networks — or run a simulation first and ask about what you just saw.",
  },
];

const API_URL =
  import.meta.env.VITE_API_URL ||
  "https://algolab1-0.onrender.com";
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        {
          role: "tutor",
          text:
            "⚠ Unable to contact the AI server. Please make sure the backend is running and deployed correctly.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <section className="max-w-3xl mx-auto px-6 py-12">
      ...
      <p className="text-dim mb-8">
        This chat uses Gemini AI running on your Render backend.
      </p>
      ...
      <div className="border border-edge rounded-xl bg-surface flex flex-col h-[480px]">
            ...
            >
              <div
                className={`max-w-[80%] rounded-lg px-4 py-2.5 text-sm leading-relaxed ${
                  m.role === "user"
                    ? "bg-cyan text-bg"
                    : "bg-surface2 border border-edge text-ink"
                }`}
              >
                {m.text}
              </div>
        ...
        <div className="border-t border-edge p-3 flex gap-2">
          ...
          <button
            onClick={send}
            disabled={loading}
            className="px-4 py-2 rounded-md bg-cyan text-bg font-mono text-xs font-semibold hover:opacity-90 transition-opacity disabled:opacity-50"
          >
            {loading ? "..." : "Send"}
          </button>
        </div>
